...
export default function ProfileStepper({ onComplete, onCancel, initialData }: ProfileStepperProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pt-12 pb-48">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/80 backdrop-blur-xl border-t border-gray-100 py-8 px-6 z-40 transition-all duration-500 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            {onCancel && (
              <button onClick={onCancel} disabled={isSubmitting} className="px-8 py-4 rounded-xl font-black text-xs uppercase tracking-widest text-gray-400 hover:bg-gray-100 transition-all">
                Cancel
              </button>
            )}
            <button onClick={handleBack} disabled={currentStep === 1 || isSubmitting} className={`flex items-center gap-2 px-8 py-4 rounded-xl font-black text-xs uppercase tracking-widest text-brand-secondary transition-all hover:bg-gray-100 ${currentStep === 1 ? 'opacity-0 pointer-events-none' : ''}`}>
              <ChevronLeft size={18} />
              Back
            </button>
          </div>

          <button onClick={handleNext} disabled={!isStepValid(currentStep) || isSubmitting} className={`flex items-center gap-4 px-12 py-4 rounded-xl font-black text-xs uppercase tracking-[0.2em] transition-all ${isStepValid(currentStep) ? 'bg-foreground text-white hover:bg-brand-accent hover:shadow-xl hover:shadow-brand-accent/20 cursor-pointer transform hover:-translate-y-0.5' : 'bg-gray-100 text-gray-400 cursor-not-allowed'} group`}>
            {isSubmitting ? (
