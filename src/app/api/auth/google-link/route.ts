...
export async function POST(req: Request) {
  try {
    ...
    const userByGoogle = await db.query.users.findFirst({
      where: eq(users.id, sql`SELECT user_id FROM accounts WHERE provider_account_id = ${google_id} AND provider = 'google'`),
    });

    // Note: Drizzle subqueries in findFirst can be tricky, let's join or do two steps
...
// Helper for subquery-like logic in Drizzle
